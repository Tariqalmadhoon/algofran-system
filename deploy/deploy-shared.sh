...
php -d disable_functions="system,exec,shell_exec,passthru,mysql_list_dbs,ini_alter,dl,symlink,link,chgrp,leak,popen,apache_child_terminate,virtual,mb_send_mail" "$(command -v composer)" install --no-dev --prefer-dist --optimize-autoloader --no-interaction --no-progress
if [[ -n "$ASSET_ARCHIVE" ]]; then    # This archive is produced by CI from the exact tested commit. Extract only
