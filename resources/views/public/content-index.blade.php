<x-public-shell :title="$heading" :description="$description">
    ...
    <section class="relative isolate overflow-hidden bg-emerald-950 pb-24 pt-20 text-white sm:pb-28 sm:pt-24">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="grid items-center gap-10 lg:grid-cols-[1fr_340px]">
                ...
                <div class="hidden lg:block" data-motion="scale">
    ...
    <section class="bg-stone-50 pb-24 pt-5 sm:pb-28">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            @if($spotlight)
                <article class="group relative z-10 -mt-16 overflow-hidden rounded-[2rem] border border-emerald-950/5 bg-white shadow-[0_35px_90px_-48px_rgba(6,78,59,.55)]" data-motion="reveal">
                    <a href="{{ route($routeName, $spotlight) }}" class="grid lg:grid-cols-[1.15fr_.85fr]">
                        <div class="relative min-h-72 overflow-hidden bg-gradient-to-br from-emerald-100 to-teal-50 lg:min-h-[430px]">
                            @if($spotlight->featuredMedia)
                                <img src="{{ $spotlight->featuredMedia->url }}" alt="{{ $spotlight->featuredMedia->alt_text ?: $spotlight->title }}" class="absolute inset-0 size-full object-cover transition duration-1000 group-hover:scale-105">
                            @else
                ...
                @if($rest->isNotEmpty())
                    <div class="mb-7 mt-14 flex items-end justify-between gap-4" data-motion="reveal"><div><p class="text-xs font-black text-amber-700">المزيد من المركز</p><h2 class="mt-1 text-2xl font-black text-emerald-950">{{ $isNews ? 'أخبار أخرى' : 'فعاليات ومحطات أخرى' }}</h2></div><span class="hidden text-xs font-bold text-slate-400 sm:block">{{ $items->total() }} مادة منشورة</span></div>
                ...
                @endif
            @else
                <div class="mx-auto max-w-2xl rounded-[2.5rem] border border-dashed border-emerald-200 bg-white px-6 py-16 text-center shadow-[0_20px_70px_-50px_rgba(6,78,59,.4)]" data-motion="scale">
            ...
            @if($items->hasPages())<div class="mt-12" data-motion="reveal">{{ $items->links() }}</div>@endif
        </div>
    </section>
</x-public-shell>
