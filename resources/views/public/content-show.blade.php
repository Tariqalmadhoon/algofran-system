    ...
    <article class="bg-stone-50">
        <header class="relative isolate overflow-hidden bg-emerald-950 pb-28 pt-16 text-white sm:pb-36 sm:pt-20">
            ...
            <div class="relative mx-auto max-w-5xl px-4 text-center sm:px-6">
                <nav class="flex items-center justify-center gap-2 text-xs font-black text-emerald-100/70" data-motion="reveal"><a href="{{ route('public.home') }}" class="transition hover:text-white">الرئيسية</a><span>/</span><a href="{{ route($backRoute) }}" class="transition hover:text-white">{{ $section }}</a></nav>
                <div class="mt-6 flex items-center justify-center gap-3" data-motion="reveal"><span class="h-px w-10 bg-amber-300/70"></span><time class="text-xs font-black text-amber-200">{{ $content->published_at?->translatedFormat('j F Y') }}</time><span class="h-px w-10 bg-amber-300/70"></span></div>
                <h1 class="mx-auto mt-6 max-w-4xl text-3xl font-black leading-[1.5] sm:text-5xl lg:text-6xl" data-motion="reveal">{{ $content->title }}</h1>
                @if($content->excerpt)<p class="mx-auto mt-6 max-w-2xl text-base leading-8 text-emerald-50/70 sm:text-lg" data-motion="reveal">{{ $content->excerpt }}</p>@endif
            </div>
        </header>

        <div class="mx-auto max-w-5xl px-4 pb-20 sm:px-6 sm:pb-28">
            @if($content->featuredMedia)
                <figure class="relative z-10 -mt-20 overflow-hidden rounded-[2rem] border-4 border-white bg-white shadow-[0_35px_100px_-45px_rgba(6,78,59,.5)] sm:-mt-28 sm:rounded-[2.75rem]" data-motion="scale">
                    <img class="aspect-[16/8.5] w-full object-cover" src="{{ $content->featuredMedia->url }}" alt="{{ $content->featuredMedia->alt_text ?: $content->title }}">
            ...
            @else
                <div class="relative z-10 -mt-16 h-2 rounded-full bg-gradient-to-l from-amber-400 via-emerald-500 to-teal-500 shadow-lg" data-motion="scale"></div>
            @endif

            <div class="mx-auto mt-10 max-w-3xl sm:mt-14">
                <div class="rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-[0_22px_70px_-50px_rgba(15,23,42,.4)] sm:p-10" data-motion="reveal">
                ...
                <div class="mt-8 flex flex-col items-center justify-between gap-4 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4 sm:flex-row" data-motion="reveal">
