        ...
        <div class="divide-y divide-slate-100">
            @forelse($notifications as $notification)
                ...
                <article class="flex gap-4 p-4 transition hover:bg-emerald-50/35 sm:p-5 {{ $notification->read_at ? 'opacity-65' : '' }}">
                    ...
                    <div class="flex shrink-0 items-center gap-1">@unless($notification->read_at)<button type="button" class="btn-ghost text-xs font-bold" wire:click="markRead('{{ $notification->id }}')">فتح</button>@endunless<button type="button" class="btn-ghost text-rose-500" @click="$dispatch('app:confirm', { title: 'حذف الإشعار', message: 'سيُحذف هذا الإشعار من سجلك نهائيًا.', confirmLabel: 'حذف الإشعار', tone: 'danger', action: () => $wire.delete('{{ $notification->id }}') })" aria-label="حذف الإشعار">×</button></div>
                </article>
            @empty
